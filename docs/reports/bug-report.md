...
`load_config()` applies `data/state/aria-state.yaml` after reading `config.yaml`; that file is allowed to overwrite the provider and model. Because it is ignored by Git and its overrides are not shown in normal startup output, a stale or experimental model survives configuration edits and makes ARIA look broken immediately after launch.

**Locations:** `src/aria/config/loader.py:278-319`, `src/aria/config/loader.py:415-416`; evidence in `data/logs/aria.log` at the 2026-09-09 startup entries.
...
**Locations:** `src/aria/ui/urwid/tui.py:631-659`; the successful-shutdown path is `src/aria/__main__.py:278-302`.
...
**Locations:** `config.yaml:260-273`, `src/aria/memory/embeddings/providers.py:79-139`.
